...
- **Entrar com o Google** (`servidor/google.mjs`): PKCE, nunca junta contas pelo e-mail, vincular pela página de conta, conta do Google define a primeira senha. Interruptor `FIO_GOOGLE=ligado` no `.env` da VPS, depois de cadastrar no console do Google os endereços `https://fiolib.duckdns.org/api/google/volta` e `https://fio.142-93-57-2.sslip.io/api/google/volta`.
